...
### 1.3 Hardcoded Paths
`scripts/generate_tension_data.py` — 8 hardcoded `C:/Users/user/...` paths (HIGH severity, pre-launch blocker). **Fixed in Phase 7.**
...
- `scripts/generate_tension_data.py` — 8 hardcoded `C:/Users/user/...` paths → env-var-based paths
...
- `"a domain-expert evaluator"` across scripts/tests/docs — 0 matches
...
`CLAUDE.md` (`C:\Users\user\Anthropic\CLAUDE.md`) lists 4 archived scripts as active in the Key Scripts table. Remove these rows:
