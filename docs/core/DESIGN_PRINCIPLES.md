...
- **The brief should surface the best version, not just the current version** — peak-state patterns alongside typical behavior
...
**Frequency is not significance.** Trading's tight feedback loop produces high-frequency conversational data. But a system break and spiral in trading does not mean a life spiral. Recurrence measures conversation frequency, not identity significance. A person's relationship to a domain — what it reveals about how they operate, not the domain-specific details — is the identity signal. The volume of data is noise.
...
- Every algorithm change in the scoring pipeline requires mandatory re-scoring of all facts — stale scores propagate through every downstream stage
...
- All clusters represent fundamental human dimensions — no hierarchy of significance
