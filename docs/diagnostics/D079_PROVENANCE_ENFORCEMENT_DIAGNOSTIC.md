...
- **a domain-expert evaluator:** 7 ungrounded (biographical details pulled from fact database, not layers)
...
- Doesn't address the a domain-expert evaluator case (her ungrounded claims came from the fact database)
...
- For private subjects (Aarik, a domain-expert evaluator): less pre-training knowledge, so less contamination risk → but the inverted-claim problem shows this isn't zero
