...
TEST_CASES = [
    ...
    ("What's my approach to risk management?", ["risk", "defined"], ["a minor health detail"]),
    ("What games do I play?", ["Deadlock", "game"], []),
    ("Tell me about myself", [], ["canadian citizen", "a minor health detail", "s2000", "iron condor"]),
]
```

The "should_NOT_contain" keywords reveal personal facts about the user: was incorrectly tagged as Canadian citizen, partner has a minor health detail, does NOT own an S2000, specific trading strategy (iron condor). Also reveals specific gaming preferences (Deadlock) and trading instruments (SPY, options).
...
```python
for term in ["Canadian citizen", "a minor health detail", "S2000", "iron condor"]:
```
...
- `docs/core/DECISIONS.md` — "Canadian citizen, a minor health detail, S2000, Razvan" (lines 550, 1212)
...
**Files:**
- `scripts/test_contradiction.py` (line 119): `"Jordan has a minor health detail-related dry skin"` — references real relationship detail
...
While these use pseudonyms ("Jordan", "Alex", "TechCo"), the specific scenarios map closely to real biographical facts. The a minor health detail/skin reference is a known the developer correction (spouse's condition incorrectly attributed to him).
...
5. `scripts/test_contradiction.py` — "Jordan has a minor health detail" test case
