"""
Round 3: Opus-as-judge evaluation of Haiku classification prompt.
Tests the LATEST CLASSIFY_PROMPT (with two new disambiguation rules added).

1. Pulls 20 random active personal-scope facts from each of the 4 fact_type categories (80 total)
2. Sends all 80 to Haiku using the LATEST CLASSIFY_PROMPT
...
OPUS_JUDGE_PROMPT = """You are evaluating fact classifications. For each fact, judge whether the assigned fact_type and commitment_depth are correct.

Definitions:
- biographical: facts about who they are, what happened to them, relationships, job history, one-off actions, capabilities, personality traits
- behavioral: RECURRING patterns of how they characteristically act — requires repeated tendency, not one-time actions or traits
- positional: what they explicitly believe, argue for, or evaluate as true/important — requires evaluative stance
- preference: what they like, are interested in (stable/general interest), choose, or gravitate toward
...
def judge_with_opus(all_facts, haiku_results):
    ...
    prompt_with_facts = OPUS_JUDGE_PROMPT + "\nFacts to evaluate:\n" + json.dumps(facts_for_judge, indent=2)
...
def main():
    ...
    print("=" * 110)
    print("ROUND 3: OPUS-AS-JUDGE EVALUATION OF HAIKU CLASSIFICATION PROMPT")
    print(f"Haiku model: {HAIKU_MODEL}")
    print(f"Judge model: {OPUS_MODEL}")
    print("Prompt version: CLASSIFY_PROMPT with two new disambiguation rules")
    print("=" * 110)

    # Print the prompt for reference
    print("\n--- CLASSIFY_PROMPT being tested ---")
    print(CLASSIFY_PROMPT[:500] + "..." if len(CLASSIFY_PROMPT) > 500 else CLASSIFY_PROMPT)
    ...
    # Check for ID format issues - Haiku might return IDs differently
    haiku_id_set = set(haiku_results.keys())
    fact_id_set = set(str(row[0]) for row in all_facts)
    matched = haiku_id_set & fact_id_set
    if len(matched) < len(all_facts):
        print(f"  WARNING: Only {len(matched)} of {len(all_facts)} fact IDs matched")
        # Try integer keys too
        int_id_lookup = {}
        for row in all_facts:
            int_id_lookup[str(row[0])] = row[0]
        # Show a few haiku IDs for debugging
        sample_haiku_ids = list(haiku_results.keys())[:5]
        sample_fact_ids = list(str(row[0]) for row in all_facts)[:5]
        print(f"  Sample Haiku IDs: {sample_haiku_ids}")
        print(f"  Sample fact IDs: {sample_fact_ids}")

    missing_haiku = [str(row[0]) for row in all_facts if str(row[0]) not in haiku_results]
    if missing_haiku:
        print(f"  WARNING: {len(missing_haiku)} facts not returned by Haiku: {missing_haiku[:10]}")

    # Step 3: Judge with Opus
    # Convert fact IDs to match haiku_results format
    # Rebuild all_facts with string IDs for consistent comparison
    all_facts_str = [(str(row[0]), row[1], row[2], row[3]) for row in all_facts]

    print(f"\n[Step 3] Sending {len(haiku_results)} facts + Haiku classifications to Opus as judge...")
    opus_results, opus_in, opus_out = judge_with_opus(all_facts_str, haiku_results)
    ...
    if missing_opus:
        print(f"  WARNING: {len(missing_opus)} facts not judged by Opus: {missing_opus[:10]}")
    ...
    # Per DB-type (original category sampled) — shows how reclassification behaves
    type_by_db_type = {}

    # Disagreement details
    disagreements = []

    for row in all_facts_str:
        fact_id, fact_text, db_type, db_depth = row
        ...
        # Per DB-type (sampled category)
        if db_type not in type_by_db_type:
            type_by_db_type[db_type] = {"correct": 0, "total": 0, "reclassified": 0}
        type_by_db_type[db_type]["total"] += 1
        if is_type_correct:
            type_by_db_type[db_type]["correct"] += 1
        if h_type != db_type:
            type_by_db_type[db_type]["reclassified"] += 1
    ...
    # Per DB-type (sampled category) — how often Haiku reclassifies and whether Opus agrees
    print("\n--- Haiku reclassification rate by original DB type ---")
    for dt in sorted(type_by_db_type.keys()):
        stats = type_by_db_type[dt]
        reclassed = stats["reclassified"]
        print(f"  {dt:<14}: {reclassed}/{stats['total']} reclassified by Haiku ({100*reclassed/stats['total']:.1f}%)")
    ...
    print(f"\n  Estimated full-corpus Haiku reclassification cost (3,956 facts): ${haiku_cost * scale_factor:.2f}")
