...
class TestCheckpointExtraction:
    ...
    def test_returns_true_with_facts(self, checkpoint_db):
        from scripts.checkpoint import checkpoint_extraction
...
class TestRunCheckpoint:
    ...
    def test_unknown_stage(self, capsys):
        from scripts.checkpoint import run_checkpoint
        with patch("scripts.checkpoint.DATABASE_FILE", MagicMock(exists=MagicMock(return_value=True))):
            with patch("scripts.checkpoint.get_db") as mock_get_db:
                mock_conn = MagicMock()
    ...
    def test_no_database(self, capsys):
        from scripts.checkpoint import run_checkpoint
        with patch("scripts.checkpoint.DATABASE_FILE", MagicMock(exists=MagicMock(return_value=False))):
